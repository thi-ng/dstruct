...
#+BEGIN_SRC clojure :noweb-ref output
  #?(:clj
     (deftype OutputStreamWrapper [^OutputStream out ^bytes buf]
       ...
       (write-uint16-le
         [_ x]
         (aset-byte bytes 0 (unchecked-byte (bit-and x 0xff)))
         (aset-byte bytes 1 (unchecked-byte (bit-and (unsigned-bit-shift-right x 8) 0xff)))
         (.write out buf 0 2)
         _)
       (write-uint16-be
         [_ x]
         (aset-byte bytes 1 (unchecked-byte (bit-and x 0xff)))
         (aset-byte bytes 0 (unchecked-byte (bit-and (unsigned-bit-shift-right x 8) 0xff)))
         (.write out buf 0 2)
         _)
       (write-uint32-le
         [_ x]
         (aset-byte bytes 0 (unchecked-byte (bit-and x 0xff)))
         (aset-byte bytes 1 (unchecked-byte (bit-and (unsigned-bit-shift-right x 8) 0xff)))
         (aset-byte bytes 2 (unchecked-byte (bit-and (unsigned-bit-shift-right x 16) 0xff)))
         (aset-byte bytes 3 (unchecked-byte (unsigned-bit-shift-right x 24)))
         (.write out 0 4)
         _)
       (write-uint32-be
         [_ x]
         (aset-byte bytes 3 (unchecked-byte (bit-and x 0xff)))
         (aset-byte bytes 2 (unchecked-byte (bit-and (unsigned-bit-shift-right x 8) 0xff)))
         (aset-byte bytes 1 (unchecked-byte (bit-and (unsigned-bit-shift-right x 16) 0xff)))
         (aset-byte bytes 0 (unchecked-byte (unsigned-bit-shift-right x 24)))
         (.write out 0 4)
         _)
       (write-float-le
         [_ x]
