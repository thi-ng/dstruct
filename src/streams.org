...
underlying native buffer and providing potentially optimized
implementations to move a type's internal data into a native buffer.
For Clojure this is intended to be used with buffers from the
=java.nio= package, or JS typed arrays in Clojurescript.
